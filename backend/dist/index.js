...
app.get('/groups', (req, res) => {
    ...
    if (typeof membersCountLt === 'number' && !Number.isNaN(membersCountLt)) {
        groups = groups.filter((g) => (g.membersCount ?? 0) < membersCountLt);
    }
    res.json(groups);
});
// Simple mock verification: accepts header Authorization: Bearer mock:<uid>
function mockVerify(req, res, next) {
...
app.post('/join-group', mockVerify, (req, res) => {
    ...
    if (!g)
        return res.status(404).json({ error: 'group not found (mock)' });
    if (!(g.isPublic || g.public))
        return res.status(403).json({ error: 'group is private (mock)' });
    if (Array.isArray(g.members) && g.members.includes(userId))
        return res.json({ ok: true, message: 'already member (mock)' });
